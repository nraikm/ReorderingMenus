...
2. Place it in KOReader's `plugins` directory as:

   ```text
   koreader/plugins/reorderingmenus.koplugin/
   ```

3. Make sure `_meta.lua`, `main.lua`, `menu_titles.lua`, `menuorder_manager.lua`, and `ui_screens.lua` are directly inside that folder.
4. Restart KOReader.

The exact location of the `koreader` data directory depends on the device and installation method. Use the same `plugins` directory as your other KOReader plugins.
